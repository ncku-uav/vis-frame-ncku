...
uint32_t currentTime=0, displayTime=0;
...
void setup()
{
  ...
  decoder.begin(FrSkySportSingleWireSerial::SOFT_SERIAL_PIN_2, &ass, &fcs, &flvss1, &flvss2, &gps, &rpm, &sp2uart, &vario);
  Serial.begin(115200);
...
/*void init_sensors(){
  for(uint8_t i = 0; i<28; i++){
    switch(sensorlist[i]){
      case(FrSkySportSensor::SensorId[0]):
      break;
    }
  }
}*/
