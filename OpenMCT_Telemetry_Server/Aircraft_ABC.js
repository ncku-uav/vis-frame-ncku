// telemetry source object for the Aircraft_ABC
...
function Aircraft_ABC() {

	
	// Initialize working Parameters and Object

	// read the keys from dictionary
	let rawDict = fs.readFileSync('../openmct/example/Aircraft_ABC/Aircraft_ABCdictionary.json')
			...
			try{ // store in history
				...
				if(this.continousLogging){					
					...
					this.asyncStringyify(message).then(function(write) {//write is the value of the resolved promise in asyncStringify
						fs.appendFile(__dirname + '/saved_logs/Aircraft_ABC_'+this.FileTimestamp+'_rawMessage.json', write, (err) => {
							if (err) {
								console.log(err);
							} 
						}) 
	...
	server.on('error', (err) => {
		console.log(`Aircraft_ABC UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("Aircraft_ABC initialized!");
};


// to update every time new data comes in
Aircraft_ABC.prototype.generateRealtimeTelemetry = function () {
...
Aircraft_ABC.prototype.generateIntervalTelemetry = function () {
...
Aircraft_ABC.prototype.notify = function (point) {
...
Aircraft_ABC.prototype.listen = function (listener) {
...
Aircraft_ABC.prototype.SetFileTimestamp = function () {
...
Aircraft_ABC.prototype.asyncStringyify = function (obj) {
...
Aircraft_ABC.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/Aircraft_ABC_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/Aircraft_ABC_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
	...
	if(command === ':exampleCommandtoPlane'){
		...
		server.send(command,14571, 'localhost')
