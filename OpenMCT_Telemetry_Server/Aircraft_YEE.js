// telemetry source object for the Aircraft_YEE
...
function Aircraft_YEE() {

	
	// Initialize working Parameters and Object

	// read the keys from dictionary
	let rawDict = fs.readFileSync('../openmct/example/Aircraft_YEE/Aircraft_YEEdictionary.json')
			...
			try{ // store in history
				...
				if(this.continousLogging){					
					...
					this.asyncStringyify(message).then(function(write) {//write is the value of the resolved promise in asyncStringify
						fs.appendFile(__dirname + '/saved_logs/Aircraft_YEE_'+this.FileTimestamp+'_rawMessage.json', write, (err) => {
							if (err) {
								console.log(err);
							} 
						}) 
	...
	server.on('error', (err) => {
		console.log(`Aircraft_YEE UDP server error:\n${err.stack}`);
		try{
    ...
    console.log("Aircraft_YEE initialized!");
};


// to update every time new data comes in
Aircraft_YEE.prototype.generateRealtimeTelemetry = function () {
...
Aircraft_YEE.prototype.generateIntervalTelemetry = function () {
...
Aircraft_YEE.prototype.notify = function (point) {
...
Aircraft_YEE.prototype.listen = function (listener) {
...
Aircraft_YEE.prototype.SetFileTimestamp = function () {
...
Aircraft_YEE.prototype.asyncStringyify = function (obj) {
...
Aircraft_YEE.prototype.command = function (command) {
	...
	if(command === ':saveHistory'){
		...
		this.asyncStringyify(this.history).then(function(write) {//write is the value of the resolved promise in asyncStringify
			fs.writeFile(__dirname + '/saved_logs/Aircraft_YEE_'+this.FileTimestamp+'_History.json', write, (err) => {
				if (err) {
					console.log(err);
				} else {
	...
	// 			fs.writeFile(__dirname + '/saved_logs/Aircraft_YEE_'+this.FileTimestam+'.json', write, (err) => {
	// 				if (err) {
	// 					throw err;
	// 				}
	...
	if(command === ':exampleCommandtoPlane'){
		...
		server.send(command,50021, '127.0.0.1')
