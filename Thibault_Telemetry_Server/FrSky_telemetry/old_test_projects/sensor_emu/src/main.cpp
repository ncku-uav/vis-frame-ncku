...
void setup()
{
  telemetry.begin(FrSkySportSingleWireSerial::SOFT_SERIAL_PIN_12, &flvss1);//, &flvss2, &fcs);
...
/*
  flvss1.setData(4.10, 4.11, 4.12, 4.13, 6.14, 4.15);
...
#else
  telemetry.begin(FrSkySportSingleWireSerial::SOFT_SERIAL_PIN_2, &ass, &fcs, &flvss1, &flvss2, &gps, &rpm, &sp2uart, &vario);
#endif
}
...
void loop()
{
  ...
  ass.setData(75.5);  // Airspeed in km/h
  ...
  rpm.setData(i/*200*/,    // Rotations per minute
  ...
  vario.setData(i/*250.5*/,  // Altitude in meters (can be negative)
